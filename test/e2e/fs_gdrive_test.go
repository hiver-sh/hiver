...
// TestFSGDriveE2E exercises the Google Drive filesystem backend end-to-end via exec:
// multiple file writes and reads, deletes, renames (same-dir and cross-dir),
// nested directories, symlinks, and directory listing.
//
// Required environment variables (OAuth2 user credentials):
//
//	HIVE_TEST_GDRIVE_FOLDER_ID      – Google Drive folder ID to use as workspace root
//	HIVE_TEST_GDRIVE_CLIENT_ID      – OAuth2 client ID
//	HIVE_TEST_GDRIVE_CLIENT_SECRET  – OAuth2 client secret
//	HIVE_TEST_GDRIVE_REFRESH_TOKEN  – OAuth2 refresh token
//
// Optional:
//
//	HIVE_TEST_GDRIVE_ACCESS_TOKEN   – short-lived access token (skips first refresh)
func TestFSGDriveE2E(t *testing.T) {
	setup.RequireStack(t)
	setup.RequireHiverCLI(t)

	folderID := os.Getenv("HIVE_TEST_GDRIVE_FOLDER_ID")
	clientID := os.Getenv("HIVE_TEST_GDRIVE_CLIENT_ID")
	clientSecret := os.Getenv("HIVE_TEST_GDRIVE_CLIENT_SECRET")
	refreshToken := os.Getenv("HIVE_TEST_GDRIVE_REFRESH_TOKEN")
	accessToken := os.Getenv("HIVE_TEST_GDRIVE_ACCESS_TOKEN")

	if folderID == "" || clientID == "" || clientSecret == "" || refreshToken == "" || accessToken == "" {
		t.Skip("HIVE_TEST_GDRIVE_FOLDER_ID, HIVE_TEST_GDRIVE_CLIENT_ID, HIVE_TEST_GDRIVE_CLIENT_SECRET, HIVE_TEST_GDRIVE_REFRESH_TOKEN, and HIVE_TEST_GDRIVE_ACCESS_TOKEN must be set")
	}

	prefix := strings.TrimSuffix(os.Getenv("HIVE_TEST_GDRIVE_PREFIX"), "/")
	if prefix == "" {
		prefix = "e2e-test"
	}
	...
	t.Run("fs_response_backend_gdrive", func(t *testing.T) {
		...
		ev := findEvent(fsResps, func(e hiverclient.SandboxEvent) bool {
			return e.Backend == "gdrive"
		})
		if ev == nil {
			t.Fatalf("no fs.response with backend=gdrive; responses:\n%s", summarizeFSResps(fsResps))
		}
	})
}
