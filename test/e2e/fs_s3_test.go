...
// TestFSS3E2E exercises the Amazon S3 filesystem backend end-to-end via exec:
...
//
//	HIVE_TEST_S3_BUCKET            – the S3 bucket name
//	HIVE_TEST_S3_ACCESS_KEY_ID     – access key ID
//	HIVE_TEST_S3_SECRET_ACCESS_KEY – secret access key
//
// Optional:
//
//	HIVE_TEST_S3_REGION        – AWS region (default: "us-east-1")
//	HIVE_TEST_S3_PREFIX        – key prefix inside the bucket (default: "e2e-test")
//	HIVE_TEST_S3_ENDPOINT      – custom endpoint for S3-compatible services (MinIO, R2, …)
//	HIVE_TEST_S3_USE_PATH_STYLE – "1"/"true" to use path-style addressing
func TestFSS3E2E(t *testing.T) {
	setup.RequireStack(t)
	setup.RequireHiverCLI(t)

	bucket := os.Getenv("HIVE_TEST_S3_BUCKET")
	accessKeyID := os.Getenv("HIVE_TEST_S3_ACCESS_KEY_ID")
	secretAccessKey := os.Getenv("HIVE_TEST_S3_SECRET_ACCESS_KEY")

	if bucket == "" || accessKeyID == "" || secretAccessKey == "" {
		t.Skip("HIVE_TEST_S3_BUCKET, HIVE_TEST_S3_ACCESS_KEY_ID, and HIVE_TEST_S3_SECRET_ACCESS_KEY must be set")
	}

	region := os.Getenv("HIVE_TEST_S3_REGION")
	if region == "" {
		region = "us-east-1"
	}

	prefix := strings.TrimSuffix(os.Getenv("HIVE_TEST_S3_PREFIX"), "/")
	if prefix == "" {
		prefix = "e2e-test"
	}
	...
	usePathStyle := false
	switch strings.ToLower(os.Getenv("HIVE_TEST_S3_USE_PATH_STYLE")) {
	case "1", "true", "yes":
		usePathStyle = true
	}

	key := fmt.Sprintf("e2e-fs-s3-%d", time.Now().UnixNano())
	config := hiverclient.SandboxConfig{
		...
		FS: []hiverclient.FileSystem{{
			...
			S3Endpoint:        os.Getenv("HIVE_TEST_S3_ENDPOINT"),
	...
	t.Run("fs_response_backend_s3", func(t *testing.T) {
		...
		if ev == nil {
			t.Fatalf("no fs.response with backend=s3; responses:\n%s", summarizeFSResps(fsResps))
		}
	})
}
