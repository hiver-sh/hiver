...
// TestFSExternalE2E exercises the `external` filesystem backend end-to-end
// via exec. An in-process host implementing the external file system HTTP
// contract (api/external_file_system.yaml) runs on the test machine; the
// sandbox reaches it through a `host-gateway` extra host, and sbxfuse turns
// every agent file operation into a call against it.
//
// Unlike the gcs/gdrive backends, this needs no credentials or external
// service — the host is stood up by the test itself, so it always runs when
// the stack is available.
func TestFSExternalE2E(t *testing.T) {
	...
	// host-gateway resolves to the host machine's IP on the container
	// network; sbxfuse's marked socket escapes the egress proxy, so the
	// host is reachable without an egress allow rule.
	...
	t.Run("fs_response_backend_external", func(t *testing.T) {
		...
		ev := findEvent(fsResps, func(e hiverclient.SandboxEvent) bool {
			return e.Backend == "external"
		})
		if ev == nil {
			t.Fatalf("no fs.response with backend=external; responses:\n%s", summarizeFSResps(fsResps))
		}
	})
}
