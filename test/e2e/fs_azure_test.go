...
// TestFSAzureE2E exercises the Azure Blob Storage filesystem backend end-to-end
// via exec: multiple file writes and reads, deletes, renames (same-dir and
// cross-dir), nested directories, symlinks, and directory listing.
//
// Required environment variables:
//
//	HIVE_TEST_AZURE_CONTAINER – the blob container name
//
// And one credential set:
//
//	HIVE_TEST_AZURE_CONNECTION_STRING – full connection string, OR
//	HIVE_TEST_AZURE_ACCOUNT + HIVE_TEST_AZURE_ACCOUNT_KEY – shared key, OR
//	HIVE_TEST_AZURE_ACCOUNT + HIVE_TEST_AZURE_SAS_TOKEN – SAS token
//
// Optional:
//
//	HIVE_TEST_AZURE_PREFIX   – key prefix inside the container (default: "e2e-test")
//	HIVE_TEST_AZURE_ENDPOINT – custom blob endpoint (e.g. the Azurite emulator)
func TestFSAzureE2E(t *testing.T) {
	setup.RequireStack(t)
	setup.RequireHiverCLI(t)

	container := os.Getenv("HIVE_TEST_AZURE_CONTAINER")
	account := os.Getenv("HIVE_TEST_AZURE_ACCOUNT")
	accountKey := os.Getenv("HIVE_TEST_AZURE_ACCOUNT_KEY")
	connStr := os.Getenv("HIVE_TEST_AZURE_CONNECTION_STRING")
	sasToken := os.Getenv("HIVE_TEST_AZURE_SAS_TOKEN")

	hasCred := connStr != "" || (account != "" && (accountKey != "" || sasToken != ""))
	if container == "" || !hasCred {
		t.Skip("HIVE_TEST_AZURE_CONTAINER and a credential (HIVE_TEST_AZURE_CONNECTION_STRING, or HIVE_TEST_AZURE_ACCOUNT with HIVE_TEST_AZURE_ACCOUNT_KEY/HIVE_TEST_AZURE_SAS_TOKEN) must be set")
	}

	prefix := strings.TrimSuffix(os.Getenv("HIVE_TEST_AZURE_PREFIX"), "/")
	if prefix == "" {
		prefix = "e2e-test"
	}
	prefix = fmt.Sprintf("%s/%d", prefix, time.Now().UnixNano())

	key := fmt.Sprintf("e2e-fs-azure-%d", time.Now().UnixNano())
	config := hiverclient.SandboxConfig{
		...
		FS: []hiverclient.FileSystem{{
			...
			AzureEndpoint:         os.Getenv("HIVE_TEST_AZURE_ENDPOINT"),
		}},
	}
	...
	t.Run("fs_response_backend_azure", func(t *testing.T) {
		...
		ev := findEvent(fsResps, func(e hiverclient.SandboxEvent) bool {
			return e.Backend == "azure"
		})
		if ev == nil {
			t.Fatalf("no fs.response with backend=azure; responses:\n%s", summarizeFSResps(fsResps))
		}
	})
}
