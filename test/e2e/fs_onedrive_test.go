...
// TestFSOneDriveE2E exercises the Microsoft OneDrive filesystem backend
// end-to-end via exec: multiple file writes and reads, deletes, renames
// (same-dir and cross-dir), nested directories, symlinks, and directory
// listing.
//
// Required environment variables (OAuth2 credentials):
//
//	HIVE_TEST_ONEDRIVE_ACCESS_TOKEN   – OAuth access token
//
// Optional (enable automatic token refresh):
//
//	HIVE_TEST_ONEDRIVE_REFRESH_TOKEN  – OAuth refresh token
//	HIVE_TEST_ONEDRIVE_CLIENT_ID      – OAuth application (client) ID
//	HIVE_TEST_ONEDRIVE_CLIENT_SECRET  – OAuth client secret
//	HIVE_TEST_ONEDRIVE_TENANT         – identity platform tenant (default: "common")
//	HIVE_TEST_ONEDRIVE_DRIVE_ID       – target a specific drive
//	HIVE_TEST_ONEDRIVE_PREFIX         – subfolder prefix (default: "e2e-test")
func TestFSOneDriveE2E(t *testing.T) {
	setup.RequireStack(t)
	setup.RequireHiverCLI(t)

	accessToken := os.Getenv("HIVE_TEST_ONEDRIVE_ACCESS_TOKEN")
	if accessToken == "" {
		t.Skip("HIVE_TEST_ONEDRIVE_ACCESS_TOKEN must be set")
	}

	prefix := strings.TrimSuffix(os.Getenv("HIVE_TEST_ONEDRIVE_PREFIX"), "/")
	if prefix == "" {
		prefix = "e2e-test"
	}
	prefix = fmt.Sprintf("%s/%d", prefix, time.Now().UnixNano())

	key := fmt.Sprintf("e2e-fs-onedrive-%d", time.Now().UnixNano())
	config := hiverclient.SandboxConfig{
		...
		FS: []hiverclient.FileSystem{{
			...
			OneDriveRefreshToken: os.Getenv("HIVE_TEST_ONEDRIVE_REFRESH_TOKEN"),
			OneDriveClientID:     os.Getenv("HIVE_TEST_ONEDRIVE_CLIENT_ID"),
			OneDriveClientSecret: os.Getenv("HIVE_TEST_ONEDRIVE_CLIENT_SECRET"),
			OneDriveTenant:       os.Getenv("HIVE_TEST_ONEDRIVE_TENANT"),
			OneDriveDriveID:      os.Getenv("HIVE_TEST_ONEDRIVE_DRIVE_ID"),
			OneDrivePrefix:       prefix,
		}},
	}
	...
	t.Run("fs_response_backend_onedrive", func(t *testing.T) {
		...
		ev := findEvent(fsResps, func(e hiverclient.SandboxEvent) bool {
			return e.Backend == "onedrive"
		})
		if ev == nil {
			t.Fatalf("no fs.response with backend=onedrive; responses:\n%s", summarizeFSResps(fsResps))
		}
	})
}
