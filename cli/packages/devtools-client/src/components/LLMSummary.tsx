...
function Bubble({
  ...
  return (
    <div className="flex flex-col rounded-lg border border-border bg-card">
      ...
      {!collapsed && (
        <div className="px-3 py-3 flex flex-col gap-3">{children}</div>
      )}
    </div>
  );
}
...
function PlainText({ text }: { text: string }) {
  return (
    <p className="text-sm leading-relaxed break-words text-foreground select-text whitespace-pre-wrap">
      {text}
    </p>
  );
}
...
function ToolUseBlock({ name, input }: { name: string; input?: unknown }) {
  const pretty = useMemo(
    () => prettyJson(input !== undefined ? JSON.stringify(input) : undefined),
    [input],
  );
  return (
    <div className="flex flex-col gap-1.5">
      <span className="text-[11px] font-medium text-muted-foreground tracking-wide">
        Tool called: <span className="text-foreground/80">{name}</span>
      </span>
      {pretty && (
  ...
  return blocks.map((blk, i) => {
    ...
    if (blk.type === "tool_use")
      return (
        <ToolUseBlock
          key={i}
          name={blk.toolName ?? "unknown"}
          input={blk.toolInput}
        />
      );
    if (blk.type === "tool_result") {
      const raw =
        typeof blk.toolResultContent === "string"
          ? blk.toolResultContent
          : JSON.stringify(blk.toolResultContent);
      const { content: pretty, isJson } = tryPretty(raw) ?? {
        content: raw ?? "",
        isJson: false,
      };
      const resolvedName =
    ...
    <Bubble role={`tools (${tools.length})`} defaultCollapsed>
      <div className="flex flex-col divide-y divide-border">
        {tools.map((tool, i) => (
          ...
          >
            <span className="text-xs font-medium text-foreground/90 select-text">
              {tool.name}
            </span>
            {tool.description && (
              <span className="text-[11px] text-muted-foreground select-text leading-relaxed whitespace-pre-wrap">
                {tool.description}
              </span>
            )}
          </div>
...
export function LLMSummary({
  ...
  summary: LLMSummaryData;
  prevSummary?: LLMSummaryData | null;
}) {
      ...
      {prevMsgs.length > 0 && (
        <div className="flex flex-col gap-3">
          ...
          >
            ...
            Previous context ({prevMsgs.length}{" "}
            {prevMsgs.length === 1 ? "message" : "messages"})
          </button>
          {prevContextExpanded &&
            prevMsgs.map(({ msg, i }) => (
