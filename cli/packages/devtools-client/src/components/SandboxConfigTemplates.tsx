        ...
        apply: (cfg) => {
          ...
          if (fs.length > 0) {
            ...
            first.acls = [
              ...((first.acls as AnyConfig[]) ?? []),
              {
                path: `${first.mount ?? "/workspace"}/secret/**`,
                access: "deny",
              },
            ];
            fs[0] = first;
        ...
        {groups.map((group, gi) => {
          ...
          return (
            <div key={gi}>
              ...
              <p
                className={`px-2 py-1 text-[10px] font-medium uppercase tracking-wider ${groupDisabled ? "text-muted-foreground/30" : "text-muted-foreground/60"}`}
              >
                {group.group}
              </p>
