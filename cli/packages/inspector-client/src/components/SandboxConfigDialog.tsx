...
function computeLineDiff(a: string, b: string): DL[] {
  ...
  function bt(i: number, j: number): void {
    if (!i && !j) return;
    if (i && j && aL[i - 1] === bL[j - 1]) {
      bt(i - 1, j - 1);
      lines.push({ type: " ", text: aL[i - 1], oldLine: 0, newLine: 0 });
    } else if (j && (!i || dp[i][j - 1] >= dp[i - 1][j])) {
      bt(i, j - 1);
      lines.push({ type: "+", text: bL[j - 1], oldLine: 0, newLine: 0 });
    } else {
      bt(i - 1, j);
      lines.push({ type: "-", text: aL[i - 1], oldLine: 0, newLine: 0 });
    }
...
function buildHunks(diff: DL[], ctx = 3): Hunk[] {
  const changed = diff.reduce<number[]>(
    (acc, l, i) => (l.type !== " " ? [...acc, i] : acc),
    [],
  );
  if (!changed.length) return [];
  const ranges: [number, number][] = [];
  let s = Math.max(0, changed[0] - ctx),
    e = Math.min(diff.length - 1, changed[0] + ctx);
  for (let i = 1; i < changed.length; i++) {
    const lo = Math.max(0, changed[i] - ctx),
      hi = Math.min(diff.length - 1, changed[i] + ctx);
    if (lo <= e + 1) {
      e = hi;
    } else {
      ranges.push([s, e]);
  ...
  return ranges.map(([s, e]) => {
    ...
    return {
      oldStart: oldLines[0]?.oldLine ?? 1,
      oldCount: oldLines.length,
      newStart: newLines[0]?.newLine ?? 1,
...
function tokenize(line: string): JT[] {
  ...
  while (i < line.length) {
    if (line[i] === '"') {
      ...
      if (nm) {
        out.push({ k: "num", v: nm[0] });
        i += nm[0].length;
        continue;
      }
      const kw = line.slice(i).match(/^(true|false|null)/);
      if (kw) {
        out.push({ k: "kw", v: kw[0] });
        i += kw[0].length;
        continue;
      }
      const last = out[out.length - 1];
      if (last?.k === "ot") last.v += line[i];
      else out.push({ k: "ot", v: line[i] });
      i++;
    }
  }
  return out;
  ...
  return (
    <>
      {tokens.map((t, i) => {
        if (t.k === "key")
          return (
            <span key={i} className="text-sky-600 dark:text-sky-300">
              {t.v}
            </span>
          );
        if (t.k === "str")
        ...
        if (t.k === "kw")
          return (
            <span key={i} className="text-blue-600 dark:text-blue-300">
              {t.v}
            </span>
          );
...
function DiffView({ oldStr, newStr }: { oldStr: string; newStr: string }) {
  const hunks = useMemo(
    () => buildHunks(computeLineDiff(oldStr, newStr)),
    [oldStr, newStr],
  );
  if (!hunks.length) {
    return (
          ...
          {hunk.lines.map((line, li) => (
            <div
              ...
              className={`flex px-4 py-px whitespace-pre ${
                line.type === "+"
                  ? "bg-green-100 text-green-900 dark:bg-green-500/15 dark:text-green-100"
                  : line.type === "-"
                    ? "bg-red-100 text-red-900 dark:bg-red-500/15 dark:text-red-100"
                    : "text-muted-foreground"
              }`}
            >
              <span className="select-none w-4 shrink-0 opacity-50">
                {line.type}
              </span>
