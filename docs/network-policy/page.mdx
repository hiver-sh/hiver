...
```python
# Hold denied requests for up to 30s so we can approve new hosts on the fly.
sandbox = await get_or_create_sandbox("approval-gated", SandboxConfig(
    image="python",
    egress_deny_wait=30,
    egress=[EgressRule(access="allow", host="api.github.com")],
))
...
async for event in sandbox.get_events_stream():
    if event.type == "egress.request" and event.access == "denied":
        if await approve(event.host):
            cfg = await sandbox.get_config()
            await sandbox.apply_config(cfg.model_copy(update={
                "egress": [*(cfg.egress or []), EgressRule(access="allow", host=event.host)],
            }))
```
...
// Hold denied requests for up to 30s so we can approve new hosts on the fly.
denyWait := 30
sandbox, err := c.GetOrCreateSandbox(ctx, "approval-gated", client.SandboxConfig{
    Image:          "python",
    EgressDenyWait: &denyWait,
    Egress:         []client.EgressRule{{Access: "allow", Host: "api.github.com"}},
})
...
for evt := range events {
    if evt.Type == "egress.request" && evt.Access == "denied" && approve(evt.Host) {
        cfg, _ := sandbox.GetConfig(ctx)
        cfg.Egress = append(cfg.Egress, client.EgressRule{Access: "allow", Host: evt.Host})
        _, _ = sandbox.ApplyConfig(ctx, *cfg)
    }
}
```
