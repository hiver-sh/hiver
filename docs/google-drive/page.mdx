...
<Callout>Combine a Drive mount with other backends in the same sandbox, for example a read-write `local` `/workspace` for scratch plus a `/drive` mount for durable output. Mount paths must be unique and non-overlapping. See [Local Files](/docs/remote-file-systems), [GCS](/docs/gcs), and [OneDrive](/docs/onedrive).</Callout>

---

Next: [OneDrive](/docs/onedrive)
