...
Launch the [Inspector](/docs/inspector), Chrome DevTools for agents, in your browser. Add `--record` to capture a session for later replay.
...
The resulting tag is what you pass as `image` in `SandboxConfig`. See [Custom Docker image](/docs/examples/custom-docker-image) for a full walkthrough.
