...
Pass `--jq <filter>` to run each event through a [jq](https://jqlang.org/) expression before it's printed. The filter is applied to one event at a time, so `select(...)` drops events, `.field` projects, and a filter that yields multiple values prints one line each:

```bash
# only log events
hiver events agent-1 --jq 'select(.type == "log")'
# just the command of each exec event
hiver events agent-1 --jq 'select(.type == "exec") | .cmd'
```
