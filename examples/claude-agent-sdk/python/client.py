...
async def main():
    ...
    for rule in config.egress or []:
        if rule.host == "api.anthropic.com" and rule.override and rule.override.headers:
            rule.override.headers["x-api-key"] = api_key

    sandbox = await get_or_create_sandbox("claude-agent-sdk-py", config)
