...
async def main():
    # The agent authenticates with your Gemini API key, read from the
    # environment. Running without it exits with an error.
    api_key = os.environ.get("GOOGLE_API_KEY")
    if not api_key:
        raise SystemExit("Set GOOGLE_API_KEY to run this example.")
    ...
    # generativelanguage.googleapis.com — never written into the sandbox itself.
    # The .hiver.json keeps only a placeholder.
    with open("agent/.hiver.json") as f:
        config = SandboxConfig.model_validate(json.load(f))
    for rule in config.egress or []:
        if (
            rule.host == "generativelanguage.googleapis.com"
            and rule.override
            and rule.override.headers
        ):
            rule.override.headers["x-goog-api-key"] = api_key
