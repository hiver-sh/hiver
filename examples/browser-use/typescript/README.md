...
- `index.ts` — provisions the `browser` image, connects Playwright over CDP, and scrapes a page.
- `package.json` — depends on `@hiver.sh/client` and `playwright-core`; `start` runs the script with `tsx`.
...
The script prints the top Hacker News titles. `playwright-core` ships no browsers of its own, which is fine here: `connectOverCDP` attaches to the sandbox's Chromium instead of launching a local one.
