...
Drive the [OpenAI Codex](https://github.com/openai/codex) CLI inside a Hiver sandbox from a TypeScript client. The prebuilt `codex` image ships the CLI; this script provisions it and runs a one-shot task with `exec`.
...
- `index.ts` — provisions the `codex` image and runs `codex exec` via `sandbox.exec`.
...
The script prints Codex's output. Edit the prompt in `index.ts` to change the task.
