...
Drive the [GitHub Copilot CLI](https://github.com/github/copilot-cli) inside a Hiver sandbox from a Python client. The prebuilt `copilot` image ships the CLI; this script provisions it and runs a one-shot task with `exec`.
...
- `main.py` — provisions the `copilot` image and runs `copilot -p` via `sandbox.exec`.
...
The script prints Copilot's output. Edit the prompt in `main.py` to change the task.
