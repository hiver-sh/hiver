// Package sandboxd is the runtime agent that wires together the MITM proxy,
// FUSE daemon, and agent workloads as a sandbox "pod". The cmd/sandboxd binary
// owns the command surface (the re-exec entrypoint and flag parsing) and hands
// the parsed Options to Run.
//
...
// Options carries the runtime knobs cmd/sandboxd parses from flags into Run.
// The flag definitions (names, defaults, and help text) live in cmd/sandboxd,
// the command surface; this struct is just their parsed form.
type Options struct {
	// APIServerPort is the port the pod's API server binds.
	APIServerPort string
	// Pack runs the pod as a multi-tenant pack host that outlives any single
	// sandbox; when false the pod hosts a single sandbox and exits with it.
	Pack bool
	// SnapshotDir is the local-disk directory for files and VM-state snapshots;
	// empty disables VM snapshots and local-disk files snapshots.
	SnapshotDir string
	// PreallocPool is the number of warm sandbox network slots to keep ready;
	// 0 disables preallocation.
	PreallocPool int
	// MaxConcurrentLaunches caps concurrent sandbox creates in flight; 0 is
	// unbounded.
	MaxConcurrentLaunches int
}

// Run boots the sandboxd pod host with the parsed options and blocks until the
// pod is torn down (signal, or — in single-sandbox mode — its sole sandbox
// going away). All of sandboxd's runtime logic lives here; cmd/sandboxd only
// parses flags and calls Run.
func Run(opts Options) {
	...
	sup := newSupervisor()
	s := api.NewSandboxServer(opts.APIServerPort, sup)
	go s.ListenAndServe()
	...
	go func() {
		...
		GenerateCaCert(caCertPath, caKeyPath)
		proxyArgs := []string{
			"-transparent",
			"-addr", proxyAddr,
			"-dns-addr", dnsAddr,
	...
	if opts.MaxConcurrentLaunches > 0 {
		sup.pack.launchSem = make(chan struct{}, opts.MaxConcurrentLaunches)
		log.Printf("sandboxd: capping concurrent launches at %d", opts.MaxConcurrentLaunches)
	}
	...
	if opts.PreallocPool > 0 {
		sup.pack.pool = newPreallocPool(sup.pack, opts.PreallocPool)
		sup.pack.pool.start()
		log.Printf("sandboxd: preallocating %d sandbox slots", opts.PreallocPool)
	}
	sup.mu.Unlock()
