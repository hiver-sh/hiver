...
def make_sandbox(client: httpx.AsyncClient) -> Sandbox:
    return Sandbox(
        SandboxRef(id="test-sandbox", endpoint="http://sandbox:8080"),
...
async def test_get_or_create_sandbox_sends_put_with_json_body() -> None:
    ...
    await get_or_create_sandbox("test-sandbox", BASE_CONFIG, readiness_timeout_s=0)
...
async def test_get_or_create_sandbox_returns_sandbox_with_correct_id_and_endpoint_on_200() -> None:
    ...
    sandbox = await get_or_create_sandbox("test-sandbox", BASE_CONFIG, readiness_timeout_s=0)
...
async def test_get_or_create_sandbox_raises_for_invalid_id() -> None:
    with pytest.raises(ValueError, match="must match"):
        await get_or_create_sandbox("invalid id!", BASE_CONFIG, readiness_timeout_s=0)
...
async def test_get_or_create_sandbox_polls_ping_before_resolving() -> None:
    ...
    sandbox = await get_or_create_sandbox(
        "test-sandbox", BASE_CONFIG, readiness_timeout_s=2.0
    )
    assert isinstance(sandbox, Sandbox)
...
async def test_shutdown_sends_post_v1_shutdown() -> None:
    ...
    async with httpx.AsyncClient() as client:
        await shutdown(make_sandbox(client))
    assert route.called
...
async def test_shutdown_resolves_on_204() -> None:
    ...
    async with httpx.AsyncClient() as client:
        result = await shutdown(make_sandbox(client))
