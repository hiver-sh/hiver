...
function parseStream(chunks: EgressChunk[]): StreamResult {
    ...
    for (const line of chunk.body.split("\n")) {
      ...
      switch (msg.type) {
        ...
        case "response.completed": {
          ...
          stopReason = (r?.incomplete_details as Record<string, unknown> | undefined) ? "incomplete" : "stop";
          break;
        }
      }
    }
  ...
  return { model, items: Object.values(items), inputTokens, outputTokens, stopReason };
}
...
export const chatgptProvider: LLMProvider = {
  matches(event) {
    return event.host === "chatgpt.com" && event.path === "/backend-api/codex/responses";
  },
  ...
  parseSummary(request, _response, chunks) {
    ...
    try { reqBody = request.body ? (JSON.parse(request.body) as ChatGPTReqBody) : null; } catch { /* ignore */ }
    ...
    const responseBlocks: LLMContentBlock[] = stream.items.map((item) => {
      if (item.type === "function_call") {
        ...
        try { toolInput = JSON.parse(item.text || "{}"); } catch { toolInput = {}; }
    ...
    return {
      ...
      messages,
      response: responseBlocks.length > 0 || stream.stopReason
