import type { EgressChunk, LLMContentBlock, LLMMessage, LLMProvider, LLMSummaryData } from "./llmProviders";
...
function parseChunks(chunks: EgressChunk[]): {
  ...
  for (const chunk of chunks) {
    ...
    for (const line of chunk.body.split("\n")) {
      ...
      try { msg = JSON.parse(jsonStr) as GeminiResponseChunk; } catch { continue; }
      ...
      if (candidate) {
        ...
        for (const part of candidate.content?.parts ?? []) {
          ...
          if (blk) {
            ...
            if (blk.type === "text" && blocks.length > 0 && blocks[blocks.length - 1].type === "text") {
              blocks[blocks.length - 1].text = (blocks[blocks.length - 1].text ?? "") + (blk.text ?? "");
            } else {
              blocks.push(blk);
            }
          }
...
export const geminiProvider: LLMProvider = {
  matches(event) {
    return event.host === "cloudcode-pa.googleapis.com" &&
      (event.path.endsWith(":streamGenerateContent") || event.path.endsWith(":generateContent"));
  },
  ...
  parseSummary(request, _response, chunks) {
    ...
    try { reqBody = request.body ? (JSON.parse(request.body) as GeminiReqBody) : null; } catch { /* ignore */ }
    ...
    const system = systemParts.map((p) => p.text ?? "").filter(Boolean).join("\n") || undefined;

    const messages: LLMMessage[] = (reqBody?.contents ?? []).map((c) => ({
      role: geminiRoleToLLM(c.role),
      content: (c.parts ?? []).map(partToBlock).filter((b): b is LLMContentBlock => b !== null),
    }));
    ...
    return {
      ...
      system,
      messages,
      response: stream.blocks.length > 0 || stream.stopReason
        ? { blocks: stream.blocks, stopReason: stream.stopReason }
        : undefined,
      usage: stream.inputTokens != null || stream.outputTokens != null
